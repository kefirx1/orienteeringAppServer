...
import pl.dev.bkwiatkowski.core.either
import pl.dev.bkwiatkowski.core.validation.*
...
class ValidateAdminPanelUserRequestUCImpl(
  ...
  override suspend fun invoke(params: ValidateAdminPanelUserRequestUC.Params): Either<DomainError, ValidationState> =
    either {
      ...
      adminPanelUserRepository.getUserByUsername(username = params.username).onRight {
        return@either ValidationState.Invalid(message = "User with this username already exists")
      }

      adminPanelUserRepository.getUserByEmail(email = params.email).onRight {
