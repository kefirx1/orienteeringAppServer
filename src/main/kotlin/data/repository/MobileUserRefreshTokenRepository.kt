...
import pl.dev.bkwiatkowski.core.either
import pl.dev.bkwiatkowski.data.dao.MobileUserRefreshTokenDAO
...
class MobileUserRefreshTokenRepositoryImpl(
  databaseProvider: DatabaseProvider,
) : MobileUserRefreshTokenRepository {

  private val database = databaseProvider.get()

  init {
    either {
      database.getRight().initTable(table = MobileUserRefreshTokenTable)
    }
  }
  ...
  override suspend fun verifyAndRevokeToken(token: String): Either<DomainError, TokenVerificationResult> = either {
    database.getRight().dbQuery {
      ...
      val tokenDao = MobileUserRefreshTokenDAO.find {
        (MobileUserRefreshTokenTable.token eq token) and
            (MobileUserRefreshTokenTable.expiresAt greater now)
      }.singleOrNull()

      if (tokenDao != null) {
        if (tokenDao.revoked) {
          val revokedAt = tokenDao.revokedAt
          val isInsideGracePeriod = revokedAt != null &&
              ChronoUnit.SECONDS.between(revokedAt, now) <= GRACE_PERIOD.inWholeSeconds
  ...
  override suspend fun revokeAllTokensForUser(userId: Int): Either<DomainError, Unit> = either {
    database.getRight().dbQuery {
      MobileUserRefreshTokenTable.update(where = { MobileUserRefreshTokenTable.userId eq userId }) { column ->
        column[revoked] = true
